  ...
  function handleResp(xhr) {
    try {
      ...
      if (count !== lastUnreadCount) {
        gmailBiffIntervals = 30 * 1000;
      } else {
        // double waiting time
        gmailBiffIntervals = Math.min(8 * 60 * 1000, gmailBiffIntervals * 2)
      }
    } catch(e) {
      gmailBiffIcon.setAttribute('style', 'filter: grayscale(100%)');
      gmailBiffText.setAttribute('value',
      ...
      gmailBiffIntervals = Math.min(16 * 60 * 1000, gmailBiffIntervals * 2)
    }
  }

  var lastCheckId = setTimeout(check, 1000);
  function check() {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
          ...
          lastCheckId = setTimeout(check, gmailBiffIntervals);
        }
      };
